...
class Solution:
    def intToRoman(self, num: int):
        ...
        for i in unit:
            ...
            if str(i)[0] == '1' or str(i)[0] == '5':    #i의 앞자리가 1,5일 때(int는 슬라이싱✕ → str으로 변환)
                output += int_roman[i]

            elif str(i)[0] == '4' or str(i)[0] == '9':    #i의 앞자리가 4,9일 때
            ...
            elif str(i)[0] == '2' or str(i)[0] == '3':    #i의 앞자리가 2,3
                q = i // keys[keys.index(i) -1]    # i가 20이면 q = 20 // 10
                output += ( int_roman[keys[keys.index(i)-1]] * q )

            elif str(i)[0] == '6' or str(i)[0] == '7' or str(i)[0] == '8':    #i의 앞자리가 6,7,8
        ...
        return ''.join(output)    #''를 기준으로 output을 합쳐 str로 반환